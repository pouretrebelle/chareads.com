# :rotating_light: Inlined copy of [`gatsby-transformer-remark`](https://www.gatsbyjs.org/packages/gatsby-transformer-remark/)

Because we're creating custom types that we want processed by remark, and with this plugin only nodes with the type `MarkdownRemark` get parsed, we need to modify the plugin to allow other types.

See line 113-116 of [`extend-node-type.js`](https://github.com/pouretrebelle/chareads.com/blob/master/plugins/gatsby-transformer-remark/extend-node-type.js#L113-L116)
